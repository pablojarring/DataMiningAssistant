...
// ShaderBackground — generado por el Shader Builder de 21st.dev ("Plasma").
// Sin dependencias: un canvas de WebGL que llena a su padre.
//
// Llegó del catálogo con el nombre `repeat-start`, que no dice nada sobre lo
// que hace; acá se llama por lo que es. El código del shader se conserva tal
// cual salió del builder — es un artefacto generado, y editarlo a mano
// significaría no poder volver a generarlo nunca más.
//
// En esta app no se usa suelto: va como capa de fondo dentro de
// `KineticShaderBackground`, debajo de la grilla cinética.
...
const UNIFORMS = {
  ...
  colorCount: 4,
  // Mas alto que el 1.5 del builder: a esa escala el patron son cuatro
  // manchones del tamano de la pantalla, y cada tarjeta cae entera dentro de
  // uno, con lo que el contraste del texto depende de donde quedo. Con celdas
  // mas chicas el fondo lee como textura y el promedio bajo cada tarjeta es
  // parejo.
  scale: 2.6,
  intensity: 0.48,
  paramA: 0.5,
  ...
  detail: 2.4,
  contrast: 0.924,
  // Tres valores cambiados respecto de lo que salio del builder, y el resto
  // intacto:
  //
  //  · `hue` estaba en 3.0369 rad (~174 grados), que es media vuelta de la
  //    rueda de color: la paleta declarada arriba es negro -> azul -> cian ->
  //    casi blanco, y esa rotacion la sacaba por el otro lado convertida en
  //    rojo oxido. Como fondo suelto se ve bien, pero acá pelea con el cian de
  //    la grilla y del acento de la interfaz. En 0, el plasma muestra los
  //    colores que su propia paleta dice tener, y las dos capas hablan el
  //    mismo idioma.
  //  · `brightness` baja porque detrás va contenido, no un titular.
  //  · `grain` baja porque a 0.35, sobre texto chico, el ruido se lee como
  //    suciedad de pantalla en vez de como textura.
  brightness: -0.34,
  saturation: 1.0,
  hue: 0.0,
  vignette: 0.61,
...
export function ShaderBackground({
  ...
  className?: string;
  /** En false dibuja un solo fotograma y no vuelve a pintar (reduced motion). */
  animated?: boolean;
}) {
  ...
  useEffect(() => {
    ...
    const compile = (type: number, src: string) => {
      ...
      gl.compileShader(s);
      // El componente original no miraba el resultado. Un shader que no compila
      // deja el canvas en negro sin decir una palabra, y desde afuera es
      // indistinguible de "el fondo está apagado a propósito" — que es
      // exactamente la hipótesis equivocada que uno persigue durante un rato.
      if (import.meta.env.DEV && !gl.getShaderParameter(s, gl.COMPILE_STATUS)) {
        console.error(
          `[ShaderBackground] el shader ${
            type === gl.VERTEX_SHADER ? "de vértices" : "de fragmentos"
          } no compiló:
${gl.getShaderInfoLog(s)}`,
    ...
    if (import.meta.env.DEV && !gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error(`[ShaderBackground] el programa no enlazó:
${gl.getProgramInfoLog(program)}`);
    ...
    const timeAnimated = animated && Math.abs(UNIFORMS.timeScale) > 0.0001;
    ...
    // Declaradas como `const` y no con `function`: una declaracion de funcion se
    // hoistea, asi que TypeScript asume que podria ejecutarse antes de los
    // `return` que descartan `canvas` y `gl` nulos, y pierde ese estrechamiento
    // dentro del cuerpo. Como expresion, el analisis de flujo se mantiene.
    const requestRender = () => {
    ...
    const render = (now: number) => {
      ...
      gl.uniform4f(
        ...
        width,
        height,
        animated ? ((now - start) / 1000) * UNIFORMS.timeScale : 0,
        UNIFORMS.colorCount,
      );
      gl.uniform4f(
        uni.space,
